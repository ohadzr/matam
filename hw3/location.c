...
int static locationNameCompare(char* location1, char* location2) {
    ...
    if (result > 0)
        return LOCATION1_BIGGER;
    if (result < 0)
        return LOCATION2_BIGGER;
...
Location static worldMapGetLocation(WorldMap world_map, char* location_name) {
    if (!world_map)
        return NULL;
    if ((!location_name) || (strlen(location_name) == EMPTY_STRING))
        return NULL;
    LIST_FOREACH( Location , current_location , world_map )
    {
        if (locationNameCompare(locationGetName(current_location),
                                location_name) == LOCATIONS_EQAUL) {
 ...
 *  set GDT*/
int static nearLocationCompareElement(NearLocationElement near_location1,
                                      NearLocationElement near_location2) {
    return nearLocationCompare((char*) near_location1, (char*) near_location2);
}
...
Location locationCreate(char* location_name) {
    if ((!location_name) || (!strlen(location_name)))
        return NULL;
...
LocationResult locationAddNearLocation(Location location,
    ...
    if ((!location) || (!near_location))
        return LOCATION_NULL_ARGUMENT;
    if (!strcmp(location->name, near_location))
        return LOCATION_ALREADY_IN_LOCATION;
    if (locationIsNearDestination(location, near_location))
        return LOCATION_ALREADY_EXIST;
...
WorldMapResult worldMapAddLocation(WorldMap world_map, Location location) {
    if ((!world_map) || (!location))
        return WORLD_MAP_NULL_ARGUMENT;
    ...
    if (result == LIST_OUT_OF_MEMORY)
        return WORLD_MAP_OUT_OF_MEMORY;
...
bool worldMapDoesLocationExist(WorldMap world_map, char* location) {
    ...
    LIST_FOREACH( Location , current_location , world_map )
    {
        if (locationNameCompare(location,
                                current_location->name)==LOCATIONS_EQAUL)
            return true;
    }
    return false;
}
...
Pokemon worldMapGetPokemonInLocation(WorldMap world_map, char* location_name) {
    if ((!world_map) || (!location_name))
        return NULL;
    Location location = worldMapGetLocation(world_map, location_name);
    if (!location)
        return NULL;
    if (worldMapDoesLocationExist(world_map,locationGetName(location)) == false)
        return NULL;
